/* ============================================================
 * AMB2API Icons - Lucide-style inline SVG library
 * 全部图标统一 viewBox=24 / stroke-width=1.75 / round caps
 *
 * 用法：
 *   <button data-icon="save">保存</button>           // 自动渲染
 *   window.icons.create('save', { size: 18 })          // 返回 SVG 节点
 *   window.icons.render(document.body)                  // 替换全部 [data-icon]
 * ============================================================ */
(function () {
  'use strict';

  /** 单图标 path 数据。每个值是一个或多个 SVG 子元素字符串。 */
  const PATHS = {
    // 导航 / 仪表盘
    ...
    'panels-left': '<rect x="3" y="3" width="18" height="18" rx="2"/><line x1="9" y1="3" x2="9" y2="21"/>',
    'panels-right': '<rect x="3" y="3" width="18" height="18" rx="2"/><line x1="15" y1="3" x2="15" y2="21"/>',
    ...
    'user-circle': '<circle cx="12" cy="12" r="10"/><path d="M16 20v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v.4"/><circle cx="12" cy="10" r="3"/>',
    ...
    'wallet': '<path d="M19 7V5a2 2 0 0 0-2-2H5a2 2 0 0 0 0 4h14a2 2 0 0 1 2 2v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5"/><circle cx="17" cy="14" r="1.5" fill="currentColor"/>',
    ...
    'message-circle': '<path d="m17 7 3 3-3 3"/><path d="M3 12c0-4.418 3.582-8 8-8h2c4.418 0 8 3.582 8 8v8.5a.5.5 0 0 1-.5.5h-7a8 8 0 0 1-8-8z"/>',
    ...
    'aurora': '<path d="M3 21c4-9 8-9 9 0"/><path d="M12 21c4-9 8-9 9 0"/><path d="M3 14c4-9 8-9 9 0"/><path d="M12 14c4-9 8-9 9 0"/>',
    ...
    'rows-3': '<rect x="3" y="3" width="18" height="18" rx="2"/><path d="M21 9H3"/><path d="M21 15H3"/>',
  ...
  function create(name, opts) {
    ...
    if (!path) {
      ...
      span.textContent = '◌';
      span.style.cssText = 'display:inline-block;width:1em;height:1em;text-align:center;color:currentColor;opacity:.5';
      span.title = 'Icon ' + name + ' 未注册';
      return span;
    }
    ...
    if (opts.class) {
      String(opts.class).split(/\s+/).forEach((c) => c && svg.classList.add(c));
    }
    svg.innerHTML = path;
    return svg;
  }

  /** 把元素的 [data-icon] 属性替换为对应 SVG */
  function render(root) {
    ...
    nodes.forEach((el) => {
      ...
      const size = sizeAttr ? parseInt(sizeAttr, 10) : (el.dataset.iconSize ? parseInt(el.dataset.iconSize, 10) : undefined);
      const svg = create(name, { size: size });
      // 如果元素是 <i> 或空容器，直接清空内容然后注入；
      // 如果元素已经有文字内容（按钮文字），把 SVG 插到首位
      if (el.firstChild) {
  ...
  window.icons = { create: create, render: render, refresh: refresh, list: list };
          ...
          m.addedNodes.forEach((node) => {
            if (node.nodeType === 1) {
              if (node.hasAttribute && node.hasAttribute('data-icon')) {
                render(node.parentNode || document);
