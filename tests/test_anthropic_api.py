...
def test_anthropic_messages_passthrough_tool_choice_from_claude_request():
    """Plan C: tool_choice is converted from Claude format, not injected."""
    client = _build_app()
    upstream_response = JSONResponse(
        content={
            "id": "resp_2",
...
def test_anthropic_messages_passthrough_max_tokens():
    """Plan C: max_tokens is passed through from Claude request, not clamped."""
    client = _build_app()
    upstream_response = JSONResponse(
        content={
            "id": "resp_4",
    ...
    assert send_mock.await_count == 1
    request_obj = send_mock.await_args.args[0]
    assert getattr(request_obj, "max_tokens") == 16000
