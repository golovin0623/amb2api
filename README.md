...
- **多租户用户令牌** ⭐：为下游用户/客户发放独立 API 令牌，支持每令牌配额、模型白名单、过期；面板"用户令牌"页或 `/api/tokens` 管理
...
除了用主口令（`API_PASSWORD`）直接访问，运营者还可以为下游用户/客户发放**独立 API 令牌**，
每个令牌可设：总调用配额、模型白名单、过期时间、启用/禁用。令牌可直接用于
`/v1/chat/completions` 与 `/v1/messages`，超配额返回 429、模型不在白名单返回 403。

管理方式二选一：
- 面板：`/ui` → "用户令牌"页（新建/禁用/清零/删除/复制）
- API（面板口令保护）：

```bash
# 新建令牌（配额 1000 次，仅允许 gpt-5）
curl -X POST http://localhost:7861/api/tokens \
  -H "Authorization: Bearer <PANEL_PASSWORD>" -H "Content-Type: application/json" \
  -d '{"name":"alice","quota":1000,"allowed_models":["gpt-5"]}'
# 返回的 token（sk-amb-...）即可发给下游使用：
curl -X POST http://localhost:7861/v1/chat/completions \
  -H "Authorization: Bearer sk-amb-xxxxx" -H "Content-Type: application/json" \
  -d '{"model":"gpt-5","messages":[{"role":"user","content":"Hi"}]}'

# 列出 / 更新 / 删除
curl http://localhost:7861/api/tokens -H "Authorization: Bearer <PANEL_PASSWORD>"
curl -X PUT http://localhost:7861/api/tokens/<token> -H "Authorization: Bearer <PANEL_PASSWORD>" \
  -H "Content-Type: application/json" -d '{"enabled":false}'
curl -X DELETE http://localhost:7861/api/tokens/<token> -H "Authorization: Bearer <PANEL_PASSWORD>"
```

相关环境变量：`TOKEN_SAVE_INTERVAL`（配额计数落盘去抖秒数，默认 5）。
