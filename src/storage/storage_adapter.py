...
class StorageAdapter:
    ...
    async def set_perf(self, key: str, value: Any) -> bool:
        """设置性能追踪数据 (Redis使用独立hash, 其他后端回退到config)"""
        self._ensure_initialized()
        if hasattr(self._backend, 'set_perf'):
            return await self._backend.set_perf(key, value)
        # 回退到 config (对于不支持独立perf存储的后端)
        return await self._backend.set_config(key, value)
    
    async def get_perf(self, key: str, default: Any = None) -> Any:
        """获取性能追踪数据"""
        self._ensure_initialized()
        if hasattr(self._backend, 'get_perf'):
            return await self._backend.get_perf(key, default)
        return await self._backend.get_config(key, default)
    
    async def delete_perf(self, key: str) -> bool:
        """删除性能追踪数据"""
        self._ensure_initialized()
        if hasattr(self._backend, 'delete_perf'):
            return await self._backend.delete_perf(key)
        return await self._backend.delete_config(key)
    
    async def get_all_perf(self) -> Dict[str, Any]:
        """获取所有性能追踪数据"""
        self._ensure_initialized()
        if hasattr(self._backend, 'get_all_perf'):
            return await self._backend.get_all_perf()
