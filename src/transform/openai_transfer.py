"""
OpenAI Transfer Module

负责将 AssemblyAI LLM Gateway 的响应（OpenAI-shape，含部分 Gemini 风格字段）
转换为 OpenAI Chat Completions 结构：非流式 ``assembly_response_to_openai``、
流式分块 ``gemini_stream_chunk_to_openai``，以及 reasoning/thought_signature 提取。
"""
