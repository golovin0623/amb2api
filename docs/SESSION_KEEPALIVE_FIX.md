...
- **续期需有证据**：cookie 滚动续期仅当服务端**确实滚动了会话 cookie/JWT** 才算成功。
  仅凭非错误状态码不够——客户端 `follow_redirects=True`，失效会话可能被 302 跳到登录页
  后返回 200。无滚动证据时返回 None 回退密码兜底，避免把已死的上游会话本地误标为新鲜、
  使后续保活一直跳过密码恢复而账号最终仍然死亡。
